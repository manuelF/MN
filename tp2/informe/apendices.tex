...
void generarRuido(vector < double >&y, int imp)
{
    ...
    if (imp == GAUSSIAN\_NOISE)
    {
        ...
        std::normal\_distribution < double >distribution(0.0, 10.0);
...
/**
 *
 * tranformar2D es M mat M'. Primero hago mat M' y despues hago M por el resultado de eso
 **/
        ...
        double > >&mat)
{
    ...
    for (int i = 0; i < n; i++)
        ...
        {
            for (int t = 0; t < n; t++)
                y[i][j] += mat[i][t] * M[j][t];	///M[j][t] porque es M'
        }
    #pragma omp parallel for
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
